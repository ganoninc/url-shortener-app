...
public record UserUrlDTO(
    @Schema(
            description = "The shortened identifier of the URL",
            example = "abc123",
            requiredMode = Schema.RequiredMode.REQUIRED)
        String shortId,
    @Schema(
            description = "The original URL",
            example = "https://www.example.com/my-article",
            requiredMode = Schema.RequiredMode.REQUIRED)
        String originalUrl,
    @Schema(
  ...
  public static UserUrlDTO from(UrlMapping urlMapping) {
    return new UserUrlDTO(
        urlMapping.getShortId(), urlMapping.getOriginalUrl(), urlMapping.getCreatedAt());
  }
}
